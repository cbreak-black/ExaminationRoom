...
\paragraph{}
There can be several kinds of control flows in a scene: One could start with the scene, and be executed stepwise until the scene and the flow ends.
...
\begin{description}
\item[Start]
The start node is unique, only one exists in each control flow.
...
\item[Wait]
Wait nodes are only useful in the main flow of a scene. They pause the flow of execution for a given amount of time.

\item[Yield]
\todo{Think about if they are needed, and what for}
